...
Public Class DaoCompras
    ...
    Public Function findComprasByCodigoCliente(codCliente As String) As List(Of Compras) Implements IDaoCompras.findComprasByCodigoCliente
        ...
        End Using
    End Function
    Public Function findCompraById(numerocompra As String) As Compras Implements IDaoCompras.findCompraById
        Using ctx As New Contexto
            Try
                Dim find = (From c In ctx.Compras
                            Where c.Numcompra = numerocompra
                            Select c).First()
                Return find
            Catch ex As Exception
                _error = ex.Message
    ...
    Public Function actualizarCompra(compra As Compras, detacompras As List(Of Det_compra)) As Boolean Implements IDaoCompras.actualizarCompra
        Using ctx As New Contexto
            Try
                ...
                Dim buscar As Compras = (From c In ctx.Compras Where c.Numcompra = compra.Numcompra Select c).First
                compra.Usuario = buscar.Usuario
                compra.Codcaja = compra.Codcaja
                'Dim buscarDet As List(Of Det_compra) = (From dt In ctx.Det_compra Where dt.Numcompra = compra.Numcompra Select dt).ToList()
                'For Each dato As Det_compra In buscarDet
                'ctx.Det_compra.DeleteOnSubmit(dato)
                'Next
                ...
                For Each deta As Det_compra In detacompras
                    Dim buscar_det As Det_compra = (From dc In ctx.Det_compra Where dc.Numcompra = deta.Numcompra And dc.Linea = deta.Linea Select dc).First
    ...
    Public Function findDetaCompra(numcompra As String) As List(Of Det_compra) Implements IDaoCompras.findDetaCompra
        ...
        End Using
    End Function
    Public Function actualizarCompraDescargue(compra As Database.Compras) As Boolean Implements IDaoCompras.actualizarCompraDescargue
        Using ctx As New Contexto
            Try
                Dim find = (From c In ctx.Compras Where c.Numcompra = compra.Numcompra Select c).Single()
    ...
    Private Function reccuperarNumeroCompra() As Integer
        Using ctx As New Contexto
            Try
                Dim cod As Integer = (From i As Ids In ctx.Ids Select i.Numcompra).Single()
                Return cod
            Catch ex As Exception
                _error = ex.Message
                Return 0
    ...
    Public Function codigoCompra() As String Implements IDaoCompras.codigoCompra
        ...
        Using ctx As New Contexto
            ...
            Try
                Dim num As Integer = ctx.ExecuteQuery(Of Integer)("select i.numcompra from ids i").First()
    ...
    Public Function anularCompra(numeroCompra As String) As Boolean Implements IDaoCompras.anularCompra
        ...
        Using ctx As New Contexto()
            ...
            Dim buscarCompra As Compras = Me.findCompraById(numeroCompra)
    ...
    Public Function crearCompra(compra As Database.Compras, detaCompras As List(Of Database.Det_compra),
                                ...
                                Optional ByVal listaAdelantos As List(Of Adelantos) = Nothing) As Boolean Implements IDaoCompras.crearCompra
        Using ctx As New Contexto()
                    ...
                    For Each dato As Adelantos In listaAdelantos
                        If adelanto > 0 Then
                            ...
                            If findAdelanto.Numcompra.Length <= 0 Then
                                findAdelanto.Numcompra = compra.Numcompra
                            Else
                                findAdelanto.Numcompra = findAdelanto.Numcompra & "; " & compra.Numcompra
                            End If
            ...
            Try
                ...
                Dim lisGeneral =
                    (From c In ctx.Compras
                     ...
                     Where c.Numcompra = numero_compra And c.Codestado <> 0
