    ...
    Private Sub cargarSucursales()
        Using ctx As New Contexto
            ...
            Catch ex As Exception
                MsgBox("Se produjo un error al cargar las sucursales. Revise la siguiente información: " & vbCr &
...
#Region "Compras generales"
    ...
    Private Sub btnBuscar_Click(sender As Object, e As EventArgs) Handles btnBuscar.Click
        Using ctx As New Contexto
            ...
            End If
            If radSaldosComprasDescargues.Checked Then
                ...
                Try
                    ...
                    End If
                    Dim buscarc =
                            ctx.ExecuteQuery(Of SaldoCompra)(
                                String.Format(
                                    "select MONTH(c.fecha) mes, sum(c.total) as valorc, sum(c.peso) as pesoc " &
                                    "from compras c where YEAR(c.fecha) = {0} " & _agencia &
                                    "group by month(c.fecha) " &
                                    "order by month(c.fecha) asc", fecha.Year)).ToList
                    Dim buscard =
                            ctx.ExecuteQuery(Of SaldoDescargue)(
                                String.Format(
                                    "select MONTH(fecha) as mes, SUM(importe) as valorD, SUM(peso) as pesoD " &
                    ...
                    ServiciosBasicos.ParametrosCrystal(txtFechaSaldos.Value, txtFechaSaldos.Value,
                                                       saldoAnioAnterior.valorS, saldoAnioAnterior.pesoS)
                    frmReportes.viewer.ReportSource = report
                    frmReportes.Show()
                    Return
                Catch ex As Exception
                    MessageBox.Show("No se pudo realizar la consulta debido a un error interno. " & vbCr & ex.Message,
                                    "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Return
                End Try
            End If
            If radOperador_general.Checked Then
                If cmbOperadorFiltrar.SelectedIndex = 0 Then
                    Dim lisCompras =
                            (From c In ctx.Compras
                             Where c.Fecha <= txtHastaGen.Value And c.Fecha >= txtDesdeGen.Value _
                                 And c.Codestado < 3
                             Select c).ToList()
                    If chkFiltrarAgencia.Checked Then
                    ...
                    If lisCompras.Count <= 0 Then
                        MsgBox("No hay datos a mostar en el rango de fechas indicado, intente nuevamente",
                               MsgBoxStyle.Information, "Compras")
                        Return
                    End If
                ...
                If cmbComprasGen.SelectedIndex = 0 Then
                    ...
                    Dim lisGeneral =
                            (From c In ctx.Compras
                             ...
                             Where c.Fecha >= txtDesdeGen.Value And c.Fecha <= txtHastaGen.Value
                             Order By CInt(c.Numcompra) Ascending
                             Select c.Numcompra, c.Codcliente, c.Fecha, c.Peso, c.Total, cli.Nombres, cli.Apellidos, c.Codagencia, c.Codmoneda).ToList()
                    ...
                    For Each dato In lisGeneral
                        suma += dato.Total
                        Dim tipoCambio = (From tc In ctx.TipoCambio Where tc.Fecha.Date = dato.Fecha.Date Select tc).Single
                        Dim vista As New ViewCompras With {
                            .Numcompra = dato.Numcompra,
                            .Codcliente = dato.Codcliente,
                            .Fecha = dato.Fecha,
                            .CodMoneda = dato.Codmoneda,
                            .Total = dato.Total,
                            .Nomcliente = dato.Nombres & " " & dato.Apellidos,
                            .TotalGeneral = suma,
                            .Peso = dato.Peso
                        }
                        Select Case dato.Codmoneda
                            Case parametros.dolares
                                vista.ImporteTipoCambio = vista.Total * tipoCambio.Tipocambio1
                            Case parametros.cordobas
                                vista.ImporteTipoCambio = vista.Total
                        End Select
                        lista.Add(vista)
                    Next
                    If lisGeneral.Count <= 0 Then
                        MsgBox("No hay datos mostrar en el rango de fechas indicado, intente nuevamente", MsgBoxStyle.Information,
                               Me.lblTitulo.Text)
                        Return
                    End If
                    Dim report As New rptComprasGeneral
                    Dim monedas = (From m In ctx.Moneda Select m).ToList
                    report.Database.Tables(1).SetDataSource(monedas)
                ...
                If cmbComprasGen.SelectedIndex = 1 Then
                    ...
                    Dim lisGeneral =
                            (From c In ctx.Compras
                             ...
                             Select
                             c.Numcompra, c.Codcliente, cli.Nombres, cli.Apellidos, dc.Peso, dc.Preciok, dc.Importe, c.Total, c.Fecha,
                             dc.Kilate, c.Codagencia, c.Codmoneda).ToList()
                    ...
                    If lisGeneral.Count <= 0 Then
                        MsgBox _
                            ("No hay datos mostrar en el rango de fechas indicado, intente nuevamente", MsgBoxStyle.Information,
                            Me.lblTitulo.Text)
                    ...
                    For Each dato In lisGeneral
                        ...
                        Dim vista As New ViewCompras With {
                            ...
                            .kilateValor = dato.Kilate,
                            .Tipocambio = tipoCambio.Tipocambio1
                        }
                        Select Case dato.Codmoneda
                            Case parametros.dolares
                                vista.ImporteTipoCambio = Decimal.Multiply(dato.Importe, tipoCambio.Tipocambio1)
                                vista.Total = Decimal.Multiply(dato.Total, tipoCambio.Tipocambio1)
                            Case parametros.cordobas
                                vista.ImporteTipoCambio = dato.Importe
                    ...
                    Try
                        Dim lisGeneral =
                                (From c In ctx.Compras
                                 ...
                                 Let
                                 pr_gramo =
                                 dc.Importe / If(dc.Peso = Decimal.Zero, Decimal.One, dc.Peso) /
                                 If(tc.Tipocambio1 = Decimal.Zero, Decimal.One, tc.Tipocambio1)
                                 Let y_value = 31.1035 * 24 / Convert.ToDecimal(dc.Kilate) / If(tc.Precio_oro = Decimal.Zero, Decimal.One, tc.Precio_oro)
                        ...
                        If lisGeneral.Count <= 0 Then
                            MsgBox _
                                ("No hay datos mostrar en el rango de fechas indicado, intente nuevamente", MsgBoxStyle.Information, lblTitulo.Text)
                            Return
                        End If
                        Dim lista As New List(Of ViewCompras)
                        Dim suma As Decimal = 0
                        For Each dato In lisGeneral
                            ...
                            Dim vista As New ViewCompras With {
                                ...
                                .TotalGeneral = suma,
                                .Importe = dato.Importe,
                                .kilateValor = Convert.ToDecimal(dato.Kilate),
                                .Peso = dato.Peso,
                                .Precio = dato.Preciok,
                                .Tipocambio = dato.Tipocambio1,
                                .Margen = Decimal.Divide(dato.Margen, 100),
                                .Porpagar = dato.PrecioKilate,
                                .Efectivo = dato.Precio_oro,
                                .Codagencia = dato.Codagencia,
                                .CodMoneda = dato.Codmoneda
                            }
                            If dato.Codmoneda = parametros.dolares Then
                                vista.ImporteTipoCambio = dato.Importe * dato.Tipocambio1
                            Else
                                vista.ImporteTipoCambio = dato.Importe
                            End If
                            lista.Add(vista)
                        ...
                        frmReportes.viewer.ReportSource = report
                    Catch ex As Exception
                        MsgBox("Debe especificar el precio del oro en el rango especificado, por favor revise si lo ha establecido en las fechas." &
        vbCr & ex.Message, MsgBoxStyle.Information, "Error")
                ...
                SelectedIndex = 3 Then
                    ...
                    If compra.Count <= 0 Then
                        ...
                        MsgBox _
                            ("No hay datos a mostar en el rango de fechas indicado, intente nuevamente", MsgBoxStyle.Information, "Compras")
            ...
            If radDetalle.Checked Then
                ...
                For Each dato In lisDetalle
                    ...
                    Numcompra = dato.Numcompra
                    vista.
                    Total = (dato.Importe / dato.Tipocambio1)
                    vista.
                    Importe = dato.Importe
                    vista.
                    Peso = dato.Peso
...
#End Region
    ...
    Private Sub frmRptComrpas_Load(sender As Object, e As EventArgs) Handles Me.Load
    ...
    Private Sub UltraButton1_Click(sender As Object, e As EventArgs) Handles UltraButton2.Click
        Me.Close()
    End Sub

    Private Sub radPorcodigo_Click(sender As Object, e As EventArgs) Handles radPorcodigo.Click
        lblBuscar.Text = "Filtrar Por codigo"
        txtFiltrar.Focus()
    End Sub

    Private Sub radPornombre_Click(sender As Object, e As EventArgs) Handles radPornombre.Click
        lblBuscar.Text = "Filtrar por nombre de cliente"
        txtFiltrar.Focus()
    End Sub

    Private Sub radPornumcompra_Click(sender As Object, e As EventArgs) Handles radPornumcompra.Click
        ...
        Using ctx As New Contexto
            ...
            If radPorcodigo.Checked Then
                Dim filtrar = (From cli In ctx.Cliente
                               Where SqlMethods.Like(cli.Codcliente, "%" & txtFiltrar.Text & "%")
            ...
            If radPornombre.Checked Then
                Dim filtrar = (From cli In ctx.Cliente
                               Where SqlMethods.Like(cli.Nombres, "%" & txtFiltrar.Text & "%")
                               Select cli.Codcliente, cli.Nombres, cli.Apellidos).ToList()
                dgvFiltrar.Rows.Clear()
                For Each dato In filtrar
                    dgvFiltrar.Rows.Add("", dato.Codcliente, dato.Nombres, dato.Apellidos)
    ...
    Private Sub btnBuscardet_Click(sender As Object, e As EventArgs) Handles btnBuscardet.Click
        Using ctx As New Contexto
            If radPorcodigo.Checked Then
                If codigoCliente.Length <= 0 Then
                    MsgBox("No se ha especificado un codigo de cliente, intente nuevamente", MsgBoxStyle.Information,
                           "Buscar por codigo")
                    Return
                End If
                ...
                Dim lisGeneral =
                        (From c In ctx.Compras
                         ...
                         Select
                         c.Numcompra, c.Codcliente, cli.Nombres, cli.Apellidos, dc.Peso, dc.Preciok, dc.Importe, c.Total,
                         c.Fecha).ToList()
                If lisGeneral.Count <= 0 Then
                    MsgBox("No hay datos a mostrar en el rango de fechas indicado, intente nuevamente",
                           MsgBoxStyle.Information, "Reporte de compras por cliente")
                    Return
                End If
                Dim lista As New List(Of ViewCompras)
                Dim suma As Decimal = 0
    ...
    Private Sub dgvFiltrar_Click(sender As Object, e As EventArgs) Handles dgvFiltrar.Click
        Try
    ...
    Private Sub txtDesdedet_ValueChanged(sender As Object, e As EventArgs) _
        Handles txtDesdedet.ValueChanged
        Using ctx As New Contexto
            Me.filtrarValores()
        End Using
    End Sub

    Private Sub txtHastadet_ValueChanged(sender As Object, e As EventArgs) _
        Handles txtHastadet.ValueChanged
        Me.filtrarValores()
    End Sub

    Private Sub radPorcodigo_CheckedChanged(sender As Object, e As EventArgs) _
        Handles radPorcodigo.CheckedChanged
        Me.filtrarValores()
    End Sub

    Private Sub radPornombre_CheckedChanged(sender As Object, e As EventArgs) _
        Handles radPornombre.CheckedChanged
        Me.filtrarValores()
    End Sub

    Private Sub radPornumcompra_CheckedChanged(sender As Object, e As EventArgs) _
        Handles radPornumcompra.CheckedChanged
        Me.filtrarValores()
    End Sub
    ...
    Private Sub radPoroperador_CheckedChanged(sender As Object, e As EventArgs) _
        Handles radPoroperador.CheckedChanged
    ...
    Private Sub radOperador_general_CheckedChanged(sender As Object, e As EventArgs) _
        Handles radOperador_general.CheckedChanged
    ...
    Private Sub radSaldosComprasDescargues_CheckedChanged(sender As Object, e As EventArgs) _
        Handles radSaldosComprasDescargues.CheckedChanged
    ...
    Private Sub chkFiltrarAgencia_CheckedChanged(sender As Object, e As EventArgs) _
        Handles chkFiltrarAgencia.CheckedChanged
        If chkFiltrarAgencia.CheckState = CheckState.Checked Then
    ...
    Private Sub radDetalleCompra_CheckedChanged(sender As Object, e As EventArgs) _
        Handles radDetalleCompra.CheckedChanged
        If radDetalleCompra.Checked Then
